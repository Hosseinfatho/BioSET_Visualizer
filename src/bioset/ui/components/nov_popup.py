...
def nov_popup_panel(state, ctrl, nov_render_window=None):
    ...
    with html.Div(
            ...
            style=(_full_style,
                   "position: fixed; " + _POS_DEFAULT + "width: 900px; height: 675px; " + _STYLE_BASE_FULL),
        ...
        with html.Div(
            ...
            with html.Div(
                    style="display: flex; align-items: center; gap: 4px; min-width: 0; color: rgba(255,255,255,0.95);"):
                with vuetify.VBtn(v_show=("!nov_auto_play", True), icon=True, x_small=True, dense=True,
                                  click=ctrl.nov_play_pause, style=_BTN_STYLE):
                    vuetify.VIcon("mdi-play", small=True)
                with vuetify.VBtn(v_show=("nov_auto_play", False), icon=True, x_small=True, dense=True,
                                  click=ctrl.nov_play_pause, style=_BTN_STYLE):
                ...
                html.Span("{{ nov_view_index_display }}",
                          style="min-width: 3ch; font-size: 0.7rem; color: rgba(255,255,255,0.95);")
                ...
                with vuetify.VBtn(v_show=("!nov_has_results", False), small=True, dense=True, click=ctrl.nov_set,
                                  style=_BTN_STYLE):
                    html.Span("Set")
                with vuetify.VBtn(v_show=("nov_has_results", False), small=True, dense=True, click=ctrl.nov_reset,
                                  style=_BTN_STYLE):
                    html.Span("Reset")
            with html.Div(style="display: flex; align-items: center; flex-shrink: 0; gap: 4px; flex-wrap: wrap;"):
                ...
                with vuetify.VBtn(icon=True, x_small=True, dense=True, click=ctrl.ov_bookmark_open_selected,
                                  style=_BTN_STYLE):
                    vuetify.VIcon("mdi-bookmark-check", small=True)
                with vuetify.VBtn(icon=True, x_small=True, dense=True, click=ctrl.ov_bookmark_save_current,
                                  style=_BTN_STYLE):
                    vuetify.VIcon("mdi-bookmark-plus-outline", small=True)
                with vuetify.VBtn(v_show=("!nov_popup_minimized", True), icon=True, x_small=True, dense=True,
                                  click="nov_popup_minimized = true", style=_BTN_STYLE):
                    vuetify.VIcon("mdi-window-minimize", small=True)
                with vuetify.VBtn(v_show=("nov_popup_minimized", False), icon=True, x_small=True, dense=True,
                                  click="nov_popup_minimized = false", style=_BTN_STYLE):
            ...
            with html.Div(
                ...
                with html.Template(v_for="(ch, idx) in nov_active_channel_items", key="ch.id"):
                    with vuetify.VListItem(dense=True,
                                           style="min-height: 26px; padding: 0 2px; background: transparent; display: flex; align-items: center; gap: 2px;"):
                        vuetify.VCheckbox(
                        ...
                        html.Span("{{ ch.name }}",
                                  style="font-size: 0.7rem; color: rgba(255,255,255,0.95); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; flex: 1; min-width: 0;")
